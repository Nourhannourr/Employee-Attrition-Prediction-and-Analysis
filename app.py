...
def predict_attrition(age, monthly_income, overtime, total_working_years,
                       ...
                       job_involvement, num_companies_worked):
    ...
    # model is the full SMOTE + XGBoost pipeline saved from the notebook.
    # SMOTE is skipped automatically at predict time, so this call is
    # identical in shape to a plain classifier.
...
st.markdown(
    "Enter the employee's details below to predict their likelihood of leaving the "
    "company. This tool uses an XGBoost model trained on the 10 most predictive features."
)
st.markdown("---")
